...
```
If `AppConfig` is in same directory as `QuickstartApplication` , any beans defined in it, will automatically be loaded into context.
Otherwise, we can use the `@Import` annotation to load it in specifically.
# Bean lifecycle
