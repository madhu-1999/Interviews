...
If a class say `Order` has two child classes `OnlineOrder` and `OfflineOrder` . When we try to autowire an instance of order class, a issue arises, which instance should be injected, `OnlineOrder` or `OfflineOrder` ? Since it cannot be resolved at runtime a `NoUniqueBeanDefinitionException` is thrown.
***Resolution***:
