...
$$\sigma^2 = E(X^2) - (E(X))^2$$
$$ =\sum P(x_i^2)\times x_i^2 \space -(\sum P(x_i)\times x_i)^2$$
## Uses
...
## Continuous Random Variables
$$f_X(x) = \int_{y\in\Omega_y } f_{X,Y}(x,y) dy$$
$$f_Y(y) = \int_{x\in\Omega_x}^\infty f_{X,Y}(x,y)dx$$
## Notes
...
$$P(A\cap B\cap C) = P(A|B\cap C)\times P(B|C)\times P(C)$$
	...
	$$P(K\cap Q\cap A) = P(A|K\cap Q)\times P(Q| K)\times P(K) = \frac{4}{50}\times \frac{4}{51}\times \frac{4}{52}$$
# Bayes Theorem
...
What is the probability that a person who gets a positive result has the condition ?
...
___Note___ :
The huge difference between the __sensitivity__ (0.95) of the test and the __positive predictive value__ (0.162) is due to the rarity of the condition being tested (0.02).
Even a small number of false positives can outnumber the few true positives.
	So if you have the condition, the test will most probably be positive but it is not enough for a definitive diagnosis
...
$P(B|A_1)$ = .9, $P(B|A_2)$ = .01, $P(B|A_3)$ = .01. (Note: .9 + .01 + .01 (= 1.) 
