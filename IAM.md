...
+ Allows you to grant limited, temporary access to AWS resources (up to 1 hr).
+ Reduces security risk by eliminating the need to store long-term access keys.
+ Each set of temporary credentials consists of three parts: 
	- **Access Key ID**: A unique public identifier for the request.
	- **Secret Access Key**: A confidential key used to sign programmatic requests.
	- **Session Token**: A mandatory additional security layer for authenticating temporary credential requests.
## APIs
...
`DecodeAuthorizationMessage`: Decode error msg when AWS API is denied.